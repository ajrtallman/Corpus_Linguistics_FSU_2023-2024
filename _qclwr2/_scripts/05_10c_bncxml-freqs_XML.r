...
dir.create("../_outputfiles/05_10_freqoutput_XML") # create a subdirectory for the frequency list files
...
for(i in corpus.files) { # access each file
   ...
   # with XML
   current.corpus.file <- xmlInternalTreeParse(current.corpus.file.flat <- scan(i, what=character(), sep="\n"))

   mw.units <- xpathSApply(current.corpus.file, # apply XPath syntax to elements of current.corpus.file
                             "//mw",            # namely the multi-word units below sentences
                             xmlValue)          # then return the data values (i.e., the multi-word units)
   mw.tags <- xpathSApply(current.corpus.file, # apply XPath syntax to elements of current.corpus.file
                          "//mw",              # namely the multi-word units below sentences
                          xmlGetAttr,          # then extract the attribute
                          "c5")                # called "c5"
   mws <- table(paste(tolower(mw.tags),
                      tolower(whitespace(mw.units)),
                      sep="\t"))
   ...
   # alternative 1 uses the following two lines
   current.corpus.file.flat <- gsub("<mw .*?</mw>", "", current.corpus.file.flat, perl=TRUE) # remove mw tags
   current.corpus.file <- xmlInternalTreeParse(current.corpus.file.flat)                     # re-import as XML

   w.units <- xpathSApply(current.corpus.file, # apply XPath syntax to elements of current.corpus.file
                          "//w",               # namely the word units below the sentences
                          xmlValue)            # then return the data values (i.e., the words)
   w.tags <- xpathSApply(current.corpus.file, # apply XPath syntax to elements of current.corpus.file
                         "//w",               # namely the word units below the sentences
                          xmlGetAttr,         # then extract the attribute
                          "c5")               # called "c5"
   # alternative 2 would be to replace the above commands with the uncommented versions of the next two lines:
   # w.units <- xpathSApply(current.corpus.file, "//w [not(parent::mw)]", xmlValue)
   # w.tags <- xpathSApply(current.corpus.file, "//w [not(parent::mw)]", xmlGetAttr, "c5")

   ws <- table(paste(tolower(w.tags),
                     tolower(whitespace(w.units)),
                     sep="\t"))

   current.file.freqs <- c(mws, ws)
   save(current.file.freqs,  # save that frequency list into
        file=paste0(         # a file ...
           "../_outputfiles/05_10_freqoutput_XML/", # whose path begins with the directory for these lists ...
           substr(basename(i), 1, 3), # whose name is the 3-character identifier of the BNC file name
           ".RData"))        # whose extension is .RData

} # end of loop

setwd("../_outputfiles/05_10_freqoutput_XML") # move to the directory with all the file-specific frequency tables
...
save(result.df, file="../10_output_XML.Rdata")
