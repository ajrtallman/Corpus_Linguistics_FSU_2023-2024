...
freq.hyphens <- result1[result1>=1000] # restrict the analysis to the 500 most frequent hyphenated forms
freq.nothings <- freq.spaces <-        # create vectors for forms with spaces and forms without hyphens
   rep(0, length(freq.hyphens))        # that also have as many elements each
...
names(freq.spaces) <- gsub("-", " ",            # give the elements of freq.spaces names by replacing the hyphen w/ spaces
                           names(freq.hyphens), # in the vector of hyphenated forms
                           perl=TRUE)           # using Perl-compatible regular expressions

search.expressions.nothings <- paste0("<w [^>]+>", names(freq.nothings), " ?</w>")
search.expressions.spaces <- paste0("<w [^>]+>", names(freq.spaces), " ?</w>")
...
for(i in corpus.files) { # access each corpus file
   ...
   freq.spaces <- freq.spaces +               # add to freq.spaces
      lapply(                                 # the result of using
         search.expressions.spaces,           # the forms with spaces
             exact.matches.2,                 # as search expressions in exact.matches.2
             corpus.vector=current.sentences, # to find in current.sentences
             gen.conc.output=FALSE)       %>% # suppress concordances, and pass it on to
      sapply("[", 1)                      %>% # sapply to extract ("[") part 1 of the output, pass it on to
   ...
   # excursus: here's the reason why dplyr and %>% can be so useful:
   # because this is the kind of nesting mess we avoid with it ...
   # freq.nothings <- freq.nothings +             # add to the current values of freq.nothings
   #    sapply(                                   # the result of applying the function length
   #       sapply(                                # to the result of extracting the first element
   #          lapply(                             # of the list that results from applying
   #             search.expressions.nothings,     # the forms without spaces or hyphens
   #             exact.matches.2,                 # the function exact.matches.2
   #             corpus.vector=current.sentences, # to search the current corpus file
   #             gen.conc.output=FALSE            # without generating the concordance output
   #          ),                                  # the end of lapplying exact.matches.2
   #          "[",                                # the extraction function of the inner sapply
   #          1                                   # what to extract of the inner sapply
   #       ),                                     # the end of the inner sapply
   #    length)                                   # the length counting and end of the outer sapply

} # end of loop

# compile the result in a single data frame
result2 <- data.frame(HYPHENATED=as.numeric(freq.hyphens),
                      SPACED=as.numeric(freq.spaces),
                      NOTHING=as.numeric(freq.nothings),
                      row.names=names(freq.hyphens))
head(result2)
save(result2, file="05_24_output.RData")
